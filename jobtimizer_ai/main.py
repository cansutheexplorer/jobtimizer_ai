...
class SyncJobtimizerService:
    def __init__(self):
        ...
        # Add search cache to prevent repeated API calls
        self._search_cache = {}
    ...
    def search_job_titles(self, query: str, limit: int = 8) -> List[Dict]:
        """Search job titles for autocomplete suggestions using vector embeddings with caching"""
        if len(query) < 2:
            return []
        ...
        if cache_key in self._search_cache:
            logger.info(f"Returning cached results for query: '{query[:20]}...'")
            return self._search_cache[cache_key]

        try:
            async def search_titles_async():
                logger.info(f"Performing new embedding search for: '{query[:20]}...'")
            ...
            results = self._run_async(search_titles_async())
